...
form.addEventListener("submit", (event: Event) => {
  ...
  if (generatedResume) {
    ...
    const shareableURL = `${window.location.origin}?user#na?me=${encodeURIComponent(username)}`;
    const generatedLink = document.querySelector(".generated-link") as HTMLDivElement;
    const shareableLink = document.querySelector(".Shareable-link") as HTMLAnchorElement;
